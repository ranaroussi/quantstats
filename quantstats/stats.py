...
def distribution(returns, compounded=True, prepare_returns=True):
    ...
    if prepare_returns:
        daily = utils._prepare_returns(daily)
...
def rolling_volatility(returns, rolling_period=126, periods_per_year=252,
                       prepare_returns=True):
    if prepare_returns:
        returns = utils._prepare_returns(returns, rolling_period)
...
def rolling_sharpe(returns, rf=0., rolling_period=126,
    ...
    if prepare_returns:
        returns = utils._prepare_returns(returns, rf, rolling_period)
...
def omega(returns, rf=0.0, required_return=0.0, periods=252):
    ...
    denom = -1. * (
        returns_less_thresh[returns_less_thresh < 0]
        .sum()
        .values[0]
    )
...
def gain_to_pain_ratio(returns, rf=0, resolution='D'):
    ...
    https://archive.is/wip/2rwFW
    """
    returns = utils._prepare_returns(returns, rf).resample(resolution).sum()
...
def cagr(returns, rf=0., compounded=True):
    ...
    In this case, rf is assumed to be expressed in yearly (annualized) terms
    """
    total = utils._prepare_returns(returns, rf)
    if compounded:
        total = comp(total)
    else:
        total = np.sum(total)
...
def r_squared(returns, benchmark, prepare_returns=True):
    ...
    _, _, r_val, _, _ = linregress(
        returns, utils._prepare_benchmark(benchmark, returns.index))
    return r_val**2
...
def information_ratio(returns, benchmark, prepare_returns=True):
    ...
    if prepare_returns:
        returns = utils._prepare_returns(returns)
    diff_rets = returns - utils._prepare_benchmark(benchmark, returns.index)
